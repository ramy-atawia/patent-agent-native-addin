#!/usr/bin/env python3
"""
Session ID Match Debug Test
Tests if there's a session ID mismatch between API and orchestrator.
"""
...
async def debug_session_id_match():
    """Debug session ID matching between API and orchestrator"""
    
    print("🔍 SESSION ID MATCH DEBUG TEST")
    print("=" * 60)
    print("Testing if there's a session ID mismatch between")
    print("API and orchestrator.")
    print("=" * 60)
    
    # Use the EXACT same session ID that the API test uses
    ...
    print(f"\n🔍 STEP 4: Test orchestrator.handle() call with EXACT same session_id")
    print("=" * 60)
...
if __name__ == "__main__":
    asyncio.run(debug_session_id_match())
